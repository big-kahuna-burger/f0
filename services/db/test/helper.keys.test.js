...
test('rotation will throw when bad alg', async t => {
  expect(async () => await rotateKey('pineapples')).rejects.toThrowError('alg must be one of ES256|RS256')
})

test('rotation will throw when config is incomplete', async t => {
  const fakeConfig = { jwks: [] }
  prisma.config.findFirst.mockResolvedValue(fakeConfig)
  expect(async () => await rotateKey('ES256')).rejects.toThrowError('you should run initializeKeys from db/helpers/keys.js first')
})
