...
describe('authorization code', () => {
  ...
  test('errors when no args are used', async ({ query, location }) => {
    try {
      await got(`http://localhost:${port}/auth`)
    } catch (error) {
...
`('returns status $status with "$query" query', async ({ query, status, matcher }) => {
    try {
      ...
      const url = `http://localhost:${port}/auth?${q}`
      await got(url)
    } catch (error) {
  ...
  ${{ client_id: 'v', redirect_uri: 'https://somerp.com/cb' }}                                          | ${303} | ${{ error: "invalid_request", error_description: "missing required parameter 'response_type'", iss: "http://idp.dev:9876" }}
  ${{ client_id: 'v', redirect_uri: 'https://somerp.com/cb', response_type: 'code' }}                   | ${303} | ${{ error: "invalid_request", error_description: "Authorization Server policy requires PKCE to be used for this request", iss: "http://idp.dev:9876" }}
  ${{ client_id: 'v', redirect_uri: 'https://somerp.com/cb', response_type: 'code', ...badchallenge }}  | ${303} | ${{ error: "invalid_request", error_description: "code_challenge must be a string with a minimum length of 43 characters", iss: "http://idp.dev:9876" }}
`('returns status $status with "$query" query', async ({ query, status, responseQuery }) => {
  ...
  test('good query produces interaction', async (ctx) => {
    prisma.oidcModel.findUnique.mockResolvedValue({
      payload: {
        client_id: 'abc',
    ...
    const q = stringify({ client_id: 'v', redirect_uri: 'https://somerp.com/cb', response_type: 'code', ...goodchallenge })
    const url = `http://localhost:${port}/auth?${q}`
    try {
